import React from 'react'
import { Document } from '@htmldocs/react'

export interface BookmarkData {
  url: string
  title: string
  tags: string[]
  folder: string
  createdAt: Date
}

interface Props {
  bookmarks: BookmarkData[]
  exportDate: string
  totalCount: number
}

function groupByFolder(bookmarks: BookmarkData[]): Record<string, BookmarkData[]> {
  return bookmarks.reduce(
    (acc, bookmark) => {
      const key = bookmark.folder
      if (!acc[key]) acc[key] = []
      acc[key].push(bookmark)
      return acc
    },
    {} as Record<string, BookmarkData[]>,
  )
}

function getDomain(url: string): string {
  try {
    return new URL(url).hostname.replace('www.', '')
  } catch {
    return ''
  }
}

export function BookmarksDocument({ bookmarks, exportDate, totalCount }: Props) {
  const grouped = groupByFolder(bookmarks)

  return (
    <Document size="A4" orientation="portrait" margin="0.7in">
      {/* ── Header ── */}
      <div
        style={{
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'space-between',
          marginBottom: '28px',
          paddingBottom: '16px',
          borderBottom: '2px solid #2563eb',
        }}
      >
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div
              style={{
                width: '32px',
                height: '32px',
                borderRadius: '8px',
                background: '#2563eb',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                color: '#fff',
                fontWeight: 'bold',
                fontSize: '16px',
                fontFamily: 'Georgia, serif',
              }}
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fillRule="evenodd" clipRule="evenodd" d="M11.9425 1.25C9.86994 1.24999 8.23716 1.24997 6.96128 1.42337C5.65122 1.60141 4.60609 1.97468 3.78484 2.80484C2.96469 3.63387 2.59692 4.68702 2.4213 6.00741C2.24998 7.29551 2.24999 8.94462 2.25 11.0411V16.139C2.24999 17.6466 2.24998 18.8402 2.34601 19.7388C2.44091 20.6269 2.64447 21.428 3.22584 21.9638C3.69226 22.3937 4.28211 22.6646 4.91165 22.7367C5.69912 22.8268 6.43439 22.4508 7.15868 21.9377C7.89163 21.4185 8.78075 20.6321 9.90254 19.6399L9.93905 19.6076C10.4591 19.1476 10.8112 18.8372 11.105 18.6224C11.3889 18.4148 11.5623 18.3397 11.7084 18.3103C11.9009 18.2715 12.0991 18.2715 12.2916 18.3103C12.4377 18.3397 12.6111 18.4148 12.895 18.6224C13.1888 18.8372 13.5409 19.1476 14.061 19.6076L14.0975 19.64C15.2193 20.6321 16.1084 21.4186 16.8413 21.9377C17.5656 22.4508 18.3009 22.8268 19.0883 22.7367C19.7179 22.6646 20.3077 22.3937 20.7742 21.9638C21.3555 21.428 21.5591 20.6269 21.654 19.7388C21.75 18.8402 21.75 17.6466 21.75 16.139V11.041C21.75 8.94462 21.75 7.2955 21.5787 6.00741C21.4031 4.68702 21.0353 3.63387 20.2152 2.80484C19.3939 1.97468 18.3488 1.60141 17.0387 1.42337C15.7628 1.24997 14.1301 1.24999 12.0575 1.25H11.9425ZM4.8512 3.85977C5.34797 3.35762 6.02251 3.06474 7.16328 2.90971C8.32645 2.75163 9.85725 2.75 12 2.75C14.1427 2.75 15.6736 2.75163 16.8367 2.90971C17.9775 3.06474 18.652 3.35762 19.1488 3.85977C19.6467 4.36303 19.9379 5.04819 20.0918 6.20518C20.2484 7.38292 20.25 8.93223 20.25 11.0975V16.0909C20.25 17.6572 20.249 18.7702 20.1625 19.5794C20.0739 20.4088 19.9104 20.72 19.7576 20.8608C19.5238 21.0763 19.2298 21.2107 18.9178 21.2464C18.7182 21.2692 18.3835 21.192 17.7083 20.7137C17.0497 20.2472 16.2211 19.5157 15.0547 18.484L15.0286 18.4609C14.5413 18.0299 14.1372 17.6725 13.7804 17.4116C13.4074 17.1388 13.0312 16.9292 12.5878 16.8398C12.1998 16.7617 11.8002 16.7617 11.4122 16.8398C10.9688 16.9292 10.5926 17.1388 10.2196 17.4116C9.86283 17.6725 9.45871 18.0299 8.97146 18.4609L8.94527 18.484C7.77887 19.5157 6.95026 20.2472 6.29165 20.7137C5.61646 21.192 5.28182 21.2692 5.08218 21.2464C4.77019 21.2107 4.47617 21.0763 4.24237 20.8608C4.08963 20.72 3.92614 20.4088 3.83752 19.5794C3.75104 18.7702 3.75 17.6572 3.75 16.0909V11.0975C3.75 8.93223 3.75156 7.38292 3.90821 6.20518C4.06209 5.04819 4.35333 4.36303 4.8512 3.85977Z" fill="white"/>
                <g transform="translate(3.6 1.4) scale(0.7)">
                  <path d="M14 7H16C18.7614 7 21 9.23858 21 12C21 14.7614 18.7614 17 16 17H14M10 7H8C5.23858 7 3 9.23858 3 12C3 14.7614 5.23858 17 8 17H10M8 12H16" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </g>
              </svg>
            </div>
            <span
              style={{
                fontSize: '22px',
                fontWeight: 'bold',
                color: '#1e3a8a',
                fontFamily: 'Georgia, serif',
                letterSpacing: '-0.3px',
              }}
            >
              LinkMine
            </span>
          </div>
          <p
            style={{
              margin: '4px 0 0 0',
              fontSize: '12px',
              color: '#6b7280',
              fontFamily: 'Arial, sans-serif',
            }}
          >
            Bookmarks Export
          </p>
        </div>

        <div style={{ textAlign: 'right', fontFamily: 'Arial, sans-serif' }}>
          <p style={{ margin: '0', fontSize: '12px', color: '#6b7280' }}>Exported on</p>
          <p style={{ margin: '2px 0 0 0', fontSize: '13px', color: '#374151', fontWeight: '600' }}>
            {exportDate}
          </p>
          <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: '#6b7280' }}>
            {totalCount} bookmark{totalCount !== 1 ? 's' : ''}
            {' · '}
            {Object.keys(grouped).length} folder{Object.keys(grouped).length !== 1 ? 's' : ''}
          </p>
        </div>
      </div>

      {/* ── Folder sections ── */}
      {Object.entries(grouped).map(([folderName, folderBookmarks]) => (
        <div key={folderName} style={{ marginBottom: '28px' }}>
          {/* Folder heading */}
          <div
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: '8px',
              marginBottom: '10px',
            }}
          >
            <span style={{ fontSize: '14px' }}>📁</span>
            <h2
              style={{
                margin: '0',
                fontSize: '15px',
                fontWeight: '700',
                color: '#1e3a8a',
                fontFamily: 'Arial, sans-serif',
                letterSpacing: '-0.2px',
              }}
            >
              {folderName}
            </h2>
            <span
              style={{
                marginLeft: '4px',
                fontSize: '11px',
                color: '#9ca3af',
                fontFamily: 'Arial, sans-serif',
              }}
            >
              ({folderBookmarks.length})
            </span>
          </div>

          {/* Bookmark rows */}
          {folderBookmarks.map((bookmark, i) => (
            <div
              key={`${bookmark.url}-${i}`}
              style={{
                display: 'flex',
                flexDirection: 'column',
                padding: '9px 12px',
                marginBottom: '5px',
                borderRadius: '6px',
                background: i % 2 === 0 ? '#f9fafb' : '#ffffff',
                borderLeft: '3px solid #3b82f6',
                pageBreakInside: 'avoid',
              }}
            >
              {/* Title row */}
              <div
                style={{
                  display: 'flex',
                  alignItems: 'flex-start',
                  justifyContent: 'space-between',
                  gap: '8px',
                }}
              >
                <a
                  href={bookmark.url}
                  style={{
                    fontSize: '13px',
                    fontWeight: '600',
                    color: '#1d4ed8',
                    textDecoration: 'none',
                    fontFamily: 'Arial, sans-serif',
                    lineHeight: '1.4',
                    flex: 1,
                  }}
                >
                  {bookmark.title || getDomain(bookmark.url)}
                </a>
                <span
                  style={{
                    fontSize: '10px',
                    color: '#9ca3af',
                    fontFamily: 'Arial, sans-serif',
                    whiteSpace: 'nowrap',
                    paddingTop: '2px',
                    flexShrink: 0,
                  }}
                >
                  {new Date(bookmark.createdAt).toLocaleDateString('en-US', {
                    year: 'numeric',
                    month: 'short',
                    day: 'numeric',
                  })}
                </span>
              </div>

              {/* URL */}
              <p
                style={{
                  margin: '2px 0 0 0',
                  fontSize: '10.5px',
                  color: '#6b7280',
                  fontFamily: 'monospace',
                  overflow: 'hidden',
                  textOverflow: 'ellipsis',
                  whiteSpace: 'nowrap',
                  maxWidth: '85%',
                }}
              >
                {bookmark.url}
              </p>

              {/* Tags */}
              {bookmark.tags.length > 0 && (
                <div
                  style={{
                    display: 'flex',
                    flexWrap: 'wrap',
                    gap: '4px',
                    marginTop: '5px',
                  }}
                >
                  {bookmark.tags.map((tag) => (
                    <span
                      key={tag}
                      style={{
                        fontSize: '10px',
                        padding: '1px 6px',
                        borderRadius: '99px',
                        background: '#dbeafe',
                        color: '#1d4ed8',
                        fontFamily: 'Arial, sans-serif',
                        fontWeight: '500',
                      }}
                    >
                      #{tag}
                    </span>
                  ))}
                </div>
              )}
            </div>
          ))}
        </div>
      ))}

      {/* ── Footer ── */}
      <div
        style={{
          marginTop: '24px',
          paddingTop: '12px',
          borderTop: '1px solid #e5e7eb',
          textAlign: 'center',
          fontFamily: 'Arial, sans-serif',
          fontSize: '10px',
          color: '#9ca3af',
        }}
      >
        Generated by LinkMine · linkmine.eliasrm.dev
      </div>
    </Document>
  )
}
