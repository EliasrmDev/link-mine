...
import { getToken, clearToken, cacheFolders, getCachedFolders, addPending } from '../shared/storage.js'
import {
  apiCheckAuth,
  apiFetchBookmarks,
  apiFetchFolders,
...
async function init() {
  ...
  if (!token || !(await apiCheckAuth(token))) {
    if (token) await clearToken() // clear expired token
    token = null
    showView('auth')
    return
  }

  // Show main view, populate in parallel
  showView('main')
  populateCurrentTab()
...
function populateFolderSelect(folders) {
  // Clear existing options except the first (No folder)
...
function renderRecent(bookmarks) {
  ...
  for (const bm of bookmarks) {
    ...
    li.innerHTML = `
      <img src="https://www.google.com/s2/favicons?domain=${domain}&sz=16" alt="" width="14" height="14" class="favicon"
           onerror="this.style.display='none'" />
      <a href="${escapeAttr(bm.url)}" target="_blank" rel="noopener noreferrer" title="${escapeAttr(bm.title)}">
